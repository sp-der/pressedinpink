...
const clearanceWraps = Array.from(
  { length: TOTAL_IMAGES },
  (_, index) => {
    ...
    return {
      number,
      src: `/wraps/clearance/clearance (${number}).png`,
      thumbnailSrc: `/wraps/clearance/thumbnails/clearance (${number}).webp`,
    };
  },
);
...
export default function ClearanceWrapsPage() {
  ...
  const showNext = useCallback(() => {
    setSelectedIndex((currentIndex) => {
      ...
      return currentIndex === clearanceWraps.length - 1
        ? 0
        : currentIndex + 1;
    });
  }, []);
  ...
  useEffect(() => {
    ...
    const nextIndex =
      selectedIndex === clearanceWraps.length - 1
        ? 0
        : selectedIndex + 1;

    const previousImage = new Image();
    previousImage.src = clearanceWraps[previousIndex].src;

    const nextImage = new Image();
    nextImage.src = clearanceWraps[nextIndex].src;
  }, [selectedIndex]);
    ...
    <main className="relative min-h-screen overflow-hidden bg-black text-white">
      ...
      <div className="relative z-10">
        ...
        <section className="mx-auto max-w-7xl px-4 py-14 sm:px-6 sm:py-20">
          <div className="mx-auto max-w-4xl rounded-[2rem] border border-red-900/80 bg-black/85 p-7 text-center shadow-2xl backdrop-blur-md sm:p-10">
            ...
            <p
              ...
              style={smokyTextShadow}
            >
              Browse discounted designs available while supplies last. Click
              any wrap to open the full-size viewer and move through all 8
              designs.
            </p>
          </div>
        </section>
        ...
        <section className="mx-auto max-w-7xl px-4 pb-20 sm:px-6">
          <div className="mb-7 rounded-2xl border border-red-900/80 bg-black/80 px-5 py-4 text-center backdrop-blur-md">
            <p
              className="text-sm font-bold text-white"
              style={smokyTextShadow}
            >
              Showing all {clearanceWraps.length} clearance wraps
            </p>
          </div>

          <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {clearanceWraps.map((wrap, index) => (
              <button
                key={wrap.src}
                type="button"
                onClick={() => setSelectedIndex(index)}
                aria-label={`Open clearance wrap ${wrap.number}`}
              ...
              >
                <div className="relative aspect-[2/1] w-full overflow-hidden">
                  <img
                    ...
                    alt={`Clearance wrap design ${wrap.number}`}
      ...
      {selectedIndex !== null && (
        <div
          role="dialog"
          aria-modal="true"
          aria-label="Clearance wrap image viewer"
        ...
        >
          ...
          <button
            ...
            aria-label="Previous clearance wrap"
          ...
          >
            <div className="relative aspect-[2/1] w-full overflow-hidden rounded-2xl bg-white">
              <img
                src={clearanceWraps[selectedIndex].src}
                alt={`Clearance wrap design ${
                  clearanceWraps[selectedIndex].number
                }`}
                draggable={false}
                className="
                  absolute left-1/2 top-1/2
            ...
            <p
              ...
              style={smokyTextShadow}
            >
              {selectedIndex + 1} / {clearanceWraps.length}
            </p>
          </div>
          ...
          <button
            ...
            aria-label="Next clearance wrap"
