...
const wraps = Array.from({ length: TOTAL_IMAGES }, (_, index) => {
  ...
  const filename = `Anime (${number})`;

  return {
    number,
    src: `${R2_WRAP_BASE_URL}/anime/originals/${filename}.png`,
    thumbnailSrc:
      `${R2_WRAP_BASE_URL}/anime/thumbnails/${filename}.webp`,
  };
});
  ...
  const renderPagination = (
    ...
    return (
      <div
        ...
        aria-label={`Anime wrap gallery ${location} pages`}
      >
        <button
          type="button"
          ...
          >
            <div className="relative aspect-[2/1] w-full overflow-hidden rounded-2xl bg-white">
              <img
                ...
                alt={`Anime wrap design ${wraps[selectedIndex].number}`}
                draggable={false}
                className="
                  absolute left-1/2 top-1/2
